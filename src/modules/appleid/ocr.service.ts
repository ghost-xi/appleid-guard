...
export class OcrService {
  ...
  async recognizeWithRetry(imageBase64: string, maxAttempts = 3): Promise<string> {
    for (let i = 0; i < maxAttempts; i++) {
      this.logger.log(`OCR attempt ${i + 1}/${maxAttempts}...`);
      const result = await this.recognizeCaptcha(imageBase64);
      if (result && result.length >= 4) {
        // 假设验证码至少 4 位
        return result;
      }
      this.logger.warn(`OCR attempt ${i + 1} failed or result too short, retrying...`);
    }
    this.logger.error('All OCR attempts failed');
    return '';
  }
}
